 ...
 */
static inline void IOMUXC_SetPinMux(uint32_t muxRegister,// muxRegister是IO复用寄存器地址，比如GPIO1_IO03 的 IO 复用寄存器SW_MUX_CTL_PAD_GPIO1_IO03的地址为 0X020E0068
                                    uint32_t muxMode, //IP复用值，也就是ALT0-ALT8，要将GPIO1_IO03设置为GPIO功能，需要设置为5
                                    uint32_t inputRegister, 
                                    /*      外设输入IO选择寄存器，有些 IO在设置为其他的复用功能以后还需要设置IO输入寄存器
                                     *  ，比如 GPIO1_IO03要复用为 UART1_RX的话还需要设置寄存器UART1_RX_DATA_SELECT_INPUT
                                     *  ，此寄存器地址为 0X020E0624
                                     */
                                    uint32_t inputDaisy, //寄存器inputRegister的值，如果 GPIO1_IO03要作为 UART1_RX引脚的话此参数就是 1
                                    uint32_t configRegister,//未使用,函数IOMUXC_SetPinConfig将会使用这个寄存器
                                    uint32_t inputOnfield) 
                                    /* IO软件输入使能，以 GPIO1_IO03 为例就是寄存器SW_MUX_CTL_PAD_GPIO1_IO03的SION位(bit4)。如果需要使能 GPIO1_IO03的软件输入功
